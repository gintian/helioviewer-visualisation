...
public class ImageLoader {
    ...
    private ImageLoader(Image image, String serverBaseURLStr, Date startDate, int zoomLevel)
    ...
    public static ImageLoader loadAdditional(Image image, BufferedImage bufferedImage, String serverBaseURLStr,
            Date startDate, Date endDate, int zoomLevel, int sideToExpand) {
        try {
    ...
    private ImageLoader(Image image, BufferedImage bufferedImage, String serverBaseURLStr, Date startDate, Date endDate,
            int zoomLevel, int sideToExpand) throws IOException, org.json.simple.parser.ParseException {
    ...
    private URL makeUrl(Date date, int i) throws MalformedURLException {
        ...
        return new URL(MessageFormat.format("{0}/api?zoomLevel={1}&dateFrom={2}", this.serverBaseURLStr, this.zoomLevel,
                TimeUtils.toString(tempDate, "yyyy-MM-dd:HH:mm:ss")));
    }
    ...
    public static HashMap<String, Integer> getApiInfo(String serverBaseURLStr)
            throws IOException, org.json.simple.parser.ParseException {
    ...
    private void extendSet() {
        ...
        if (!this.tileBuffer.getMap().isEmpty()) {
            ...
            if (te.getValue().getStartDate().getTime() < this.diagram.getStartDate().getTime()
                    || te.getValue().getEndDate().getTime() > this.diagram.getEndDate().getTime()) {
                ...
                if (this.sideToExpand == LEFT) {
                    ...
                    endDate = TimeUtils.addTime(this.diagram.getEndDate(),
                            -Image.pixelToTime(this.tileWidth, this.zoomLevel));
                    g.drawImage(te.getValue().getBufferedImage(), 0, 0, null);
                    g.drawImage(this.diagram.getBufferedImage(), this.tileWidth, 0, null);
                } else {
                    startDate = TimeUtils.addTime(this.diagram.getStartDate(),
                            Image.pixelToTime(this.tileWidth, this.zoomLevel));
