...
public class DiagramAPI extends HttpServlet {
    ...
    private void handleRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        logger.log(Level.INFO, "Diagram API called with parameters: {0}",
                new Object[] { getParameterString(request.getParameterMap()) });

        response.setContentType("text/html");

        DiagramAPIParameters parameters = new DiagramAPIParameters();
        try {
            parameters.setDateFrom(TimeUtils.fromString(request.getParameter(DiagramAPIParameters.PARAM_DATE_FROM),
            ...
            logger.log(Level.INFO, "Diagram API request canceled due to missing / invalid parameters, sending HTTP 400",
                    new Object[] {});
    ...
    private void getImage(DiagramAPIParameters parameters, HttpServletResponse response) throws Exception {
        ...
        long currentTimePerImage = CacheRenderer.getTimePerImage(parameters.getZoomLevel());
        ...
        long imageOffset = (currentStartDate.getTime() - TimelinesDB.DB_START_DATE.getTime()) / currentTimePerImage;
        Date actualStartDate = new Date(TimelinesDB.DB_START_DATE.getTime() + imageOffset * currentTimePerImage);
    ...
    private BufferedImage getFromCache(Date date, int zoomLevel) {
        ...
        logger.log(Level.INFO, "Getting image from Cache. Zoom level: {0}, Date: {1}",
                new Object[] { zoomLevel, date });
    ...
    private void writeAPIDoc(HttpServletResponse response) throws IOException {
        ...
        printWriter.println("<ul><li>'" + DiagramAPIParameters.PARAM_DATE_FROM + "' ("
                + DiagramAPIParameters.DATE_FORMAT + ")</li>");
