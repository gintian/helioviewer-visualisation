	...
	public Set<GoesSxrLeaf> getGoesSxrLeafs(Instant startTimestamp, DateMidnight currentDateMidnight) {
		...
		while (downloadTrials < MAX_GOESNR) {
			try {
				...
				final ReadableInstant maxStartTimestamp = TimeUtils.getMaxReadableInstant(startTimestamp,
						getStartDateMidnight());
...
//				logger.warn(e.toString());
	 ...
	 * @param url
	 * @param startTimestamp only return GoesSxrLeafs with timestamp which are AFTER or EQUALS the startTimestamp.
	 * @param endTimestamp only return GoesSxrLeafs with timestamp which are BEFORE or EQUALS the endTimestamp.
	 * @return parsed GoesSxrLeafs.
	 * @throws IOException
	 */
	private Set<GoesSxrLeaf> downloadGoesSxrLeafs(URL url, ReadableInstant startTimestamp, ReadableInstant endTimestamp)
			throws IOException {
